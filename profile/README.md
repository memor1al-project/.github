<div align="center">

<img src="https://raw.githubusercontent.com/memor1al-project/.github/main/logo.jpeg" alt="The Memorial Project" width="180" style="border-radius:50%;" />

<img src="https://img.shields.io/badge/🕯️-The_Memorial_Project-b8860b?style=for-the-badge&labelColor=0e0c0a" alt="The Memorial Project" />

# The Memorial Project

### Preserving *memory.* Defying forgetting.

We use artificial intelligence to ensure Holocaust testimonies, survivor stories, and historical records endure — not as static archives, but as living, accessible knowledge.

**[Visit Our Full Interactive Page](https://memor1al-project.github.io/)**

---

*Never Again Begins With Never Forgetting*

---

</div>

## Our Mission

The Memorial Project is a nonprofit at the intersection of **AI and Holocaust remembrance**. We partner with institutions worldwide — archives, museums, education centers — to transform fragile testimonies into permanent, searchable, educational resources.

As the generation of survivors passes, the urgency of our work compounds. Every transcript, photograph, and recorded voice is a thread connecting the present to one of history's darkest chapters. We are here to hold those threads.

<div align="center">

**Archive & Digitize** | **Animate & Humanize** | **Educate & Distribute**
:---: | :---: | :---:
Ingest, structure, and semantically enrich fragile historical records across institutions worldwide | Generate first-person narratives, AI-assisted memorial sites, and interactive testimony experiences | Build curriculum tools and produce content that reaches students from elementary through university

---

</div>

## Memorial Stories
*Preserving voices. Honoring lives.*

<table>
<tr>
<td width="50%">

### [The Memorial Project](https://memorial-project.org/)
**Platform · Main Hub**

The central platform for AI-powered Holocaust testimony preservation. Aggregating archival records, survivor narratives, and educational tools across institutions worldwide — including **Yad Vashem**, the **USHMM**, and the **Arolsen Archives**. Built for researchers, educators, and the public alike.

</td>
<td width="50%">

### [Regina Feldman](https://vhec-regina-feldman.web.app/)
**Memorial · VHEC Collaboration**

A dedicated memorial for Holocaust survivor Regina Feldman, created in partnership with the **Vancouver Holocaust Education Centre**. Her testimony, photographs, and life story — preserved for future generations, told in her own voice.

</td>
</tr>
<tr>
<td width="50%">

### [Voices of Resilience — Anita's Story](https://memorial-heritage-project.web.app/stories)
**Memorial · Survivor Testimony**

Preserving the stories of Holocaust survivors — their courage, their loss, and their enduring legacy. Anita's testimony is a window into the resilience of those who endured the unimaginable and chose to bear witness for future generations.

</td>
<td width="50%">

### [Veteran Memorial](https://veteran-memorial-site-d8829.web.app/)
**Memorial · Veterans**

Honoring the veterans who bore witness and fought back. A memorial platform preserving the stories of **Canadian men and women who served in World War II** — interweaving military history with personal testimony.

</td>
</tr>
</table>

---

## Educational Projects
*Understanding history. Confronting ignorance.*

<table>
<tr>
<td width="50%">

### [Tell Institute Course](https://tell-institute-course.web.app/)
**Education · Curriculum**

An AI-enhanced educational course developed with **The Tell Institute**, exploring Jewish identity, history, and culture. Interactive, testimony-grounded, and designed to meet students where they are.

</td>
<td width="50%">

### [7 Minutes · 7 Stories](https://seven-stories-jewish-identity.web.app/)
**Education · Jewish Identity**

Seven bite-sized stories exploring the richness and diversity of Jewish identity. Designed for educators and learners alike, each narrative offers an accessible entry point into Jewish culture, history, and lived experience.

</td>
</tr>
<tr>
<td width="50%">

### [Return to the Land](https://zionism-decolonization.web.app/)
**Education · History**

Exploring Zionism as the world's only successful indigenous decolonization movement. A historical and educational resource examining the Jewish people's deep-rooted connection to their ancestral homeland and the modern return that redefined nationhood.

</td>
<td width="50%">

### [One Region, Many Stories](https://one-region-stories.web.app/)
**Education · Geopolitics**

All 24 nations rose from the same imperial collapse after World War I. But their paths since independence went in very different directions. An interactive exploration of how — and why — nations shaped by the same history arrived at such different futures.

</td>
</tr>
<tr>
<td width="50%">

### [Soviet Anti-Zionism](https://soviet-antizionism.web.app/)
**Education · Primary Sources**

The weaponization of the word "Zionist" — a 45-minute lesson through images, video, and primary sources. Tracing how Soviet propaganda systematically distorted language to delegitimize Jewish self-determination, with echoes still felt today.

</td>
<td width="50%">
</td>
</tr>
</table>

---

<div align="center">

## Why This Matters

</div>

We are living through a turning point. The last generation of Holocaust survivors and World War II veterans is passing. Within a decade, there will be no living witnesses left to share their stories firsthand. At the same time, misinformation and historical denial are on the rise.

**Technology gives us the means to fight back.**

- **Reconstruct fragmented family histories** from scattered records across dozens of countries
- **Create interactive, immersive experiences** that make history personal and tangible
- **Provide educators with modern tools** to teach the next generation effectively
- **Build permanent digital archives** that will outlast physical documents

---

<div align="center">

## Technology & Infrastructure

</div>

Our technical foundation combines large language models, multimodal AI, graph databases, and cloud archival infrastructure to ensure these records remain accessible — and meaningful — indefinitely.

`LangChain` · `LlamaIndex` · `Claude AI` · `Neo4j` · `Cognee DB` · `DeepFace` · `Firebase` · `Google Cloud` · `Vercel` · `HeyGen` · `React` · `Arolsen Archives API` · `USHMM API` · `Yad Vashem` · `Europeana` · `DPLA` · `FFmpeg`

---

<div align="center">

### Join the work of remembrance.

Whether you're a researcher, developer, educator, or institution — there is a place for you in this mission.

**[Visit the Platform](https://memorial-project.org/)** · **[Full Interactive Page](https://memor1al-project.github.io/)** · **[Get in Touch](mailto:liorp@memorial-project.org)**

---

> *"To forget the dead would be akin to killing them a second time."*
> — **Elie Wiesel**

---

*Built with purpose. Powered by remembrance.*

© 2025 The Memorial Project · Nonprofit

</div>
